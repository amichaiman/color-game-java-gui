...
public class Shape {
	...
	private void initShape(Dimension maxDimShape) {
		Point pointShape = choosePosition(maxDimShape);		//random point
		Dimension dimShape = chooseDimension(pointShape, maxDimShape);	//random dimension < max > min
	...
	private void initShape(Point pointShape, Dimension dimShape) {
		color = chooseColor();						//random color
		type = ShapeType.values()[randInt(2)];	//random shape
